...
- When a per-student share/message dialog offers WhatsApp, it should also offer SMS as a parallel channel (and vice versa) — don't ship only one messaging channel where multiple are applicable. The user treats a missing SMS option next to WhatsApp (or the reverse) as an omission to fix, not an optional enhancement. Confidence: 0.70
...
- When a percentage surcharge/fee applies on top of a base amount (e.g., a recharge package's 1.8% charge), surface the full decomposition — base amount + charge amount = payable — rather than only the final total, and compute the charge from the base amount (never from an already-adjusted total). Confidence: 0.65
...
- Outbound SMS/WhatsApp messages should stay short and concise — when a verbose detail breakdown (e.g., a per-item "Due Details" section) makes the message too long, remove it and keep only the core message. Confidence: 0.80
# Workflow
