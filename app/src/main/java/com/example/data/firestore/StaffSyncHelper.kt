...
object StaffSyncHelper {
            ...
            try {
                firestore.document(staffPath(staff.instituteId, staff.id)).set(
                    mapOf(
                        ...
                        "staffCategory" to staff.staffCategory,
                        "salaryType" to staff.salaryType,
                        "perClassRate" to staff.perClassRate,
                        "perHourRate" to staff.perHourRate,
                        "subjects" to (staff.subjects ?: ""),
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                StaffFirestoreData(
                    ...
                    staffCategory = doc.getString("staffCategory") ?: "administration",
                    salaryType = doc.getString("salaryType") ?: "monthly",
                    perClassRate = (doc.get("perClassRate") as? Number)?.toDouble() ?: 0.0,
                    perHourRate = (doc.get("perHourRate") as? Number)?.toDouble() ?: 0.0,
            ...
            try {
                staffCollection(instituteId).forEachDocumentPage { documents ->
                    ...
                    StaffEntity(
                        ...
                        perClassRate = (doc.get("perClassRate") as? Number)?.toDouble() ?: 0.0,
                        perHourRate = (doc.get("perHourRate") as? Number)?.toDouble() ?: 0.0,
                        subjects = doc.getString("subjects")?.takeIf { it.isNotBlank() },
    ...
    private fun DocumentSnapshot.toStaffEntity(instituteId: String): StaffEntity? {
        ...
        return StaffEntity(
            ...
            staffCategory = getString("staffCategory") ?: "administration",
            salaryType = getString("salaryType") ?: "monthly",
            perClassRate = (get("perClassRate") as? Number)?.toDouble() ?: 0.0,
            perHourRate = (get("perHourRate") as? Number)?.toDouble() ?: 0.0,
            subjects = getString("subjects")?.takeIf { it.isNotBlank() },
